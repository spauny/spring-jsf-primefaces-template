...
@Component
@Scope("session")
public class AndroidBrowserExcelAnalyzer implements Serializable {
    ...
    private Iterator<Row> getExcelRows(ExcelFileVO excelFIleVO, int sheetNumber) {
        try {
            ...
            Logger.getLogger(AndroidBrowserExcelAnalyzer.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }

    public void analyzeExcel(ExcelFileVO excelFIleVO, Iterator<Row> rowIterator) {
        this.globalManager.getAndroidBrowsersInfo().clear();
...
//        Iterator<Row> rowIterator = getExcelRows(excelFIleVO, sheetNumber);
        ...
        while (rowIterator.hasNext()) {
            ...
            if (this.globalManager.getAndroidBrowsersInfo().size() > rowCounter) {
                browserInfoVO = this.globalManager.getAndroidBrowsersInfo().get(rowCounter);
            } else {
                browserInfoVO = new BrowserInfoVO();
                this.globalManager.getAndroidBrowsersInfo().add(browserInfoVO);
            }
        ...
        this.filteredAndroidBrowserList = new ArrayList<>(this.globalManager.getAndroidBrowsersInfo());
    }
    ...
    private void callSetMethodForColumn(String columnHeader, Object value, BrowserInfoVO browserInfoVO) {
        try {
            ...
            Logger.getLogger(AndroidBrowserExcelAnalyzer.class.getName()).log(Level.SEVERE, "header: " + columnHeader + " value: " + value, ex);
        }
    }
